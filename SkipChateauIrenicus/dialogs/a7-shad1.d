...
IF ~~ thief.3
  ...
  ++ @2009 /* I'll take the key. Thanks. */ + thief.bye
  ++ @2010 /* No need. I'd rather find the way out by myself. */ + thief.rejected
END

IF ~~ thief.bye
  SAY @2011 /* Here you go. Now if you'll excuse me. I've got to hunt down a treacherous mage. */
  IF ~~ DO ~SetGlobal("A7-ShortcutEnabled", "GLOBAL", 1)
            SetGlobal("A7-Cutscene", "MYAREA", 2)
            GiveItemCreate("MISC4G", LastTalkedToBy, 0, 0, 0)
            ActionOverride("Chest8", DestroyItem("MISC4G"))
            EscapeAreaObject("JailPortal")~ SOLVED_JOURNAL @5000 EXIT
END

IF ~~ thief.rejected
  SAY @2012 /* I don't care anyway. I've got to hunt down a treacherous mage. */
  IF ~~ DO ~SetGlobal("A7-Cutscene", "MYAREA", 2)
            EscapeArea()~ SOLVED_JOURNAL @5001 EXIT
END
